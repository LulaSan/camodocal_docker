...
if [ "$1" = "--run-rm" ]; then
	...
	${containerName}:${containerTag} /bin/bash -c "cd /root/camodocal/build/bin && ./$2 && chmod -R a+rw /root/output_data/*"
...
if [ "$1" = "--run-keep" ]; then
	...
	--volume=`pwd`/input_data:/root/input_data:rw \
	--volume=`pwd`/output_data:/root/output_data:rw \
	${containerName}:${containerTag} /bin/bash -c "cd /root/camodocal/build/bin && /bin/bash"
	rc=$?; if [[ $rc != 0 && $rc != 1 ]]; then exit $rc; fi
